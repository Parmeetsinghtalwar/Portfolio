...
export const PROJECTS: Project[] = [
  {
    ...
    status: 'Production',
    description:
      'Content Phase monorepo (React/Vite + FastAPI): GPT-4o copy, cloud images (DALL-E 3 / Fal), and direct open-source media via ComfyUI — Z-Image-Turbo for stills, Wan for video — plus RAG, calendar, persona agents, UGC, and OAuth publish to five networks + Telegram.',
    highlights: [
      'ComfyUI path: Z-Image-Turbo (Apache-2.0 6B) + Wan 2.1/2.2 video — open-weight, no API markup',
      'Cloud path: DALL-E 3, Fal Flux, persona IP-Adapter when premium speed matters',
      '500K+ posts · 12M+ assets · OAuth-only publish',
      'Monorepo: SPA, FastAPI, APScheduler worker, Docker Compose',
    ],
    stack: [
      'Monorepo',
      'FastAPI',
  ...
  },
  {
    ...
    status: 'Production',
    description:
      'Production omni-channel support SaaS: NestJS monorepo (platform) + FastAPI L1 agent service for multi-brand commerce on WhatsApp, Telegram, and Instagram — RAG catalog, vision matching, multilingual inbox, and human takeover.',
    highlights: [
      'NestJS 11 monorepo (API, RBAC, webhooks, BullMQ) + separate FastAPI agent + React operator dashboard',
      ...
      'Docker + Terraform deploy paths for production',
    ],
    stack: [
      'NestJS 11',
      'Monorepo',
  ...
  },
  {
    ...
    status: 'Production',
    description:
      'Taleweaver powers Bookgen from a single monorepo: React/Vite frontend, FastAPI backend, GPU llm-service (Qwen2.5-3B + Fitzgerald LoRA), and offline training scripts. Outline → chapter WebSocket streaming → EPUB/PDF export.',
    highlights: [
      'Bookgen monorepo — web, api, llm-service, and LoRA training pipeline in one repository',
      'Docker Compose deploy: frontend, FastAPI backend, GPU llm-service',
      'Fitzgerald-style LoRA — 5 ePub sources → 3,762 training examples (offline training pipeline)',
  ...
  },
  {
    ...
    highlights: [
      'Lives in the Bookgen monorepo — training scripts + fitzgerald_lora/ adapter beside llm-service',
